...
RUN git clone https://github.com/forgetopchain/go-forgetop go-forgetop/
RUN cd go-forgetop && make gath

# Pull gath into another container
FROM alpine:latest

RUN apk add --no-cache ca-certificates
COPY --from=builder /go/go-forgetop/build/bin/gath /usr/local/bin/
